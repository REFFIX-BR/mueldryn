...
#pragma warning restore VSTHRD103 // Call async methods when in an async method
    ...
    private async Task<IHost> CreateHostAsync(string[] args)
    {
        ...
        if (!args.Contains("-skipdataupdates") && !args.Contains("-demo"))
        {
            await this.ApplyMandatoryDataUpdatesAsync(host.Services).ConfigureAwait(false);
        }
    ...
    /// <summary>
    /// Installs the mandatory configuration updates before the servers go live, so a freshly deployed
    /// build never runs against configuration data of an older version. Use -skipdataupdates to opt out.
    /// </summary>
    private async Task ApplyMandatoryDataUpdatesAsync(IServiceProvider services)
    {
        try
        {
            if (services.GetService<IPersistenceContextProvider>() is not { } contextProvider
                || services.GetService<PlugInManager>() is not { } plugInManager)
            {
                return;
            }

            var updateService = new DataUpdateService(contextProvider, plugInManager);
            var available = await updateService.DetermineAvailableUpdatesAsync().ConfigureAwait(false);
            ...
            this._logger.Information("Applying {count} mandatory configuration update(s)...", mandatory.Count);
            var progress = new Progress<(UpdateVersion CurrentUpdatingVersion, bool IsCompleted)>(state =>
            {
                if (state is { IsCompleted: true, CurrentUpdatingVersion: not UpdateVersion.Undefined })
                {
                    this._logger.Information("Configuration update applied: {version}", state.CurrentUpdatingVersion);
                }
            });

            await updateService.ApplyUpdatesAsync(mandatory, progress).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            this._logger.Error(ex, "Could not apply the mandatory configuration updates.");
        }
    }
