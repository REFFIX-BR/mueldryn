...
public class DeActivateMagicEffectPlugIn : IActivateMagicEffectPlugIn, IDeactivateMagicEffectPlugIn
{
    private static readonly ReadOnlyDictionary<AttributeDefinition, EffectItemConsumption.EffectType> EffectTypeMapping = new(
        new Dictionary<AttributeDefinition, EffectItemConsumption.EffectType>
        {
            ...
            { Stats.AttackDamageIncrease, EffectItemConsumption.EffectType.Damage },
            { Stats.MinimumPhysBaseDmg, EffectItemConsumption.EffectType.PhysicalDamage },
            { Stats.WizardryAttackDamageIncrease, EffectItemConsumption.EffectType.WizardryDamage },
            { Stats.BonusExperienceRate, EffectItemConsumption.EffectType.ExperienceRate },
            { Stats.MoneyAmountRate, EffectItemConsumption.EffectType.DropRate },
            { Stats.MovementSpeed, EffectItemConsumption.EffectType.Mobility },
        });
    ...
    {
        return this.SendMagicEffectStatusAsync(effect, affectedObject, true, effect.Definition.SendDuration ? effect.RemainingDuration : TimeSpan.Zero);
    }
    ...
    private async ValueTask SendMagicEffectStatusAsync(MagicEffect effect, IAttackable affectedObject, bool isActive, TimeSpan duration)
    {
        ...
        if (isActive && affectedObject == this._player)
        {
            ...
            // Without a duration, the client shows the time of its own item table, which is wrong for
            // the buffs of the item shop, so those are sent as an item consumption effect as well.
            if (effectType is not null || duration > TimeSpan.Zero)
            {
                ...
                await this._player.Connection.SendEffectItemConsumptionAsync(origin, effectType ?? default, action, (uint)duration.TotalSeconds, (byte)effect.Definition.Number).ConfigureAwait(false);
                effectWasSent = true;
            }
        }
    ...
    private static EffectItemConsumption.EffectType? GetEffectType(MagicEffect effect)
    {
