...
public class ItemSerializerExtended : IItemSerializer
{
    ...
    public int SerializeItem(Span<byte> target, Item item)
    {
        ...
        {
            var excellent = GetExcellentByte(item);
            excellent |= GetFenrirByte(item);
            if (item.ExcellentBonusStrength > 0 || item.ExcellentBonusEnergy > 0)
            {
                excellent |= ExcellentBonusFlag;
            }

            targetStruct.Excellent = excellent;
            targetStruct.ExcellentRarity = GetExcellentRarityByte(item);
            if ((excellent & ExcellentBonusFlag) != 0)
            {
                targetStruct.ExcellentBonusStrength = item.ExcellentBonusStrength;
                targetStruct.ExcellentBonusEnergy = item.ExcellentBonusEnergy;
            }
        }
    ...
    public Item DeserializeItem(Span<byte> array, GameConfiguration gameConfiguration, IContext persistenceContext)
    {
        ...
        if (itemStruct.Options.HasFlag(OptionFlags.HasExcellent))
        {
            ...
            else
            {
                ...
                ApplyExcellentRarityByte(itemStruct.ExcellentRarity, item);
            }

            if ((itemStruct.Excellent & ExcellentBonusFlag) != 0)
            {
                item.ExcellentBonusStrength = itemStruct.ExcellentBonusStrength;
                item.ExcellentBonusEnergy = itemStruct.ExcellentBonusEnergy;
            }
        }
    ...
    private OptionFlags GetOptionFlags(Item item)
    {
        ...
        if (item.ItemOptions.Any(o => o.ItemOption?.OptionType == ItemOptionTypes.Excellent
            ...
            || item.ExcellentBonusStrength > 0
            || item.ExcellentBonusEnergy > 0)
        {
    ...
    private readonly ref struct ItemStruct(Span<byte> data)
    {
        ...
        public byte ExcellentRarity
        {
            get => this.Options.HasFlag(OptionFlags.HasExcellent) ? this._data[this.ExcellentRarityIndex] : default;
            set => this._data[this.ExcellentRarityIndex] = value;
        }

        public byte ExcellentBonusStrength
        {
            get => this.HasExcellentBonusStats ? this._data[this.ExcellentBonusIndex] : default;
            set => this._data[this.ExcellentBonusIndex] = value;
        }

        public byte ExcellentBonusEnergy
        {
            get => this.HasExcellentBonusStats ? this._data[this.ExcellentBonusIndex + 1] : default;
            set => this._data[this.ExcellentBonusIndex + 1] = value;
        }

        private bool HasExcellentBonusStats =>
            this.Options.HasFlag(OptionFlags.HasExcellent) && (this.Excellent & ExcellentBonusFlag) != 0;
        ...
        public int Length
        {
            get
            {
                ...
                {
                    size++; // excellent flags
                    size++; // rarity pack
                    if ((this._data[this.ExcellentIndex] & ExcellentBonusFlag) != 0)
                    {
                        size += 2; // strength + energy
                    }
                }
        ...
        private int ExcellentRarityIndex => this.ExcellentIndex + 1;

        private int ExcellentBonusIndex => this.ExcellentRarityIndex + 1;

        private int PostExcellentIndex
        {
            get
            {
                if (!this.Options.HasFlag(OptionFlags.HasExcellent))
                ...
                if ((this.Excellent & ExcellentBonusFlag) != 0)
                {
                    index += 2;
                }
