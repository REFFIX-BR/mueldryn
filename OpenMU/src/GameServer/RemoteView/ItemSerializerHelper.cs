...
public static class ItemSerializerHelper
{
    ...
    /// <summary>
    /// Flag in the excellent option byte for Excellent Bonus Options (Strength/Energy follow as extra bytes).
    /// Bits 0-5 are excellent options. Bit 0x40 is reserved. Bit 0x80 = has Exc Bonus stats.
    /// A rarity-pack byte always follows the excellent byte (2 bits per option, Number order).
    /// </summary>
    public const byte ExcellentBonusFlag = 0x80;
    ...
    /// <summary>
    /// Packs excellent-option rarities (stored in <see cref="ItemOptionLink.Level"/>) into one byte.
    /// Two bits per option, in ascending <see cref="IncreasableItemOption.Number"/> order.
    /// </summary>
    public static byte GetExcellentRarityByte(Item item)
    {
        byte result = 0;
        var shift = 0;
        foreach (var link in item.ItemOptions
                     .Where(o => o.ItemOption?.OptionType == ItemOptionTypes.Excellent)
                     .OrderBy(o => o.ItemOption!.Number))
        {
            result |= (byte)((link.Level & 0x3) << shift);
            shift += 2;
            if (shift >= 8)
            {
                break;
    ...
    /// <summary>
    /// Applies packed rarities onto excellent option links (Level = rarity tier).
    /// </summary>
    public static void ApplyExcellentRarityByte(byte rarityByte, Item item)
    {
        var shift = 0;
        foreach (var link in item.ItemOptions
                     .Where(o => o.ItemOption?.OptionType == ItemOptionTypes.Excellent)
                     .OrderBy(o => o.ItemOption!.Number))
        {
            link.Level = (byte)((rarityByte >> shift) & 0x3);
            shift += 2;
            if (shift >= 8)
            {
                break;
