...
public class Player : AsyncDisposable, IBucketMapObserver, IAttackable, IAttacker, ITrader, IPartyMember, IRotatable, IHasBucketInformation, ISupportWalk, IMovable, ILoggerOwner<Player>
{
    ...
    {
        var miniGame = this.CurrentMiniGame;

        // Stay on the instance map. A full RemoveAsync fires MiniGame.ObjectRemoved,
        // which used to drop CurrentMiniGame, warp the player to public map 69,
        // and (solo) cancel the run — disposing every wave monster.
        var stayOnSameMap = miniGame is not null
            || object.Equals(this.CurrentMap?.Definition, gate.Map);

        if (!await this.TryRemoveFromCurrentMapAsync(stayOnSameMap).ConfigureAwait(false))
        {
            return;
        }
        ...
        {
            this.CurrentMap = miniGame.Map;
            if (this.ViewPlugIns.GetPlugIn<IRespawnAfterDeathPlugIn>() is { } dungeonRespawnPlugIn)
            {
                await dungeonRespawnPlugIn.RespawnAsync().ConfigureAwait(false);
            }
    ...
    public async ValueTask ClientReadyAfterMapChangeAsync()
    {
        ...
        {
            if (this.SelectedCharacter.CurrentMap is { Number: 69 or 70 or 71 })
            {
                var homeMap = this.SelectedCharacter.CharacterClass?.HomeMap;
                var spawn = homeMap?.ExitGates.FirstOrDefault(g => g.IsSpawnGate)
                    ?? homeMap?.ExitGates.FirstOrDefault();
                if (spawn is not null)
                {
                    await this.PlaceAtGateAsync(spawn).ConfigureAwait(false);
                }
            }
        ...
        await this.SendActiveMagicEffectsAsync().ConfigureAwait(false);
    }
    ...
    private async ValueTask AddExperienceCoreAsync(int experience, IAttackable? killedObject)
    {
        ...
        while (remainingExperience > 0)
        {
            ...
            var level = (int)this.Attributes[Stats.Level];
            // Heal characters stuck after a buggy reset (Experience left at 0 while Level > 1).
            if (level > 0 && level < expTable.Length && this.SelectedCharacter!.Experience < expTable[level])
            {
                this.SelectedCharacter.Experience = expTable[level];
            }
    ...
    private async ValueTask OnDeathAsync(IAttacker? killer)
    {
        ...
        async Task RespawnAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                this.SetReclaimableAttributesToMaximum();
                var respawnGate = this.CurrentMiniGame?.GetPlayerRespawnGate()
                    ?? await this.GetSpawnGateOfCurrentMapAsync().ConfigureAwait(false);
                await this.RespawnAtAsync(respawnGate).ConfigureAwait(false);
    ...
    /// <summary>
    /// Sends the running magic effects to the game client again. The client forgets them when it
    /// (re-)creates the character, which happens after a login and after each map change.
    /// </summary>
    private async ValueTask SendActiveMagicEffectsAsync()
    {
        var effects = this.MagicEffectList.ActiveEffects.Values
            .Where(effect => effect.Id < MagicEffectsList.InvisibleEffectStartIndex)
            .ToList();

        foreach (var effect in effects)
        {
            await this.InvokeViewPlugInAsync<IActivateMagicEffectPlugIn>(p => p.ActivateMagicEffectAsync(effect, this)).ConfigureAwait(false);
        }
    }
    ...
    private async ValueTask OnPlayerEnteredWorldAsync()
    {
        ...
        // Collections / Soul Max HP must be applied before reclaim clamp, or CurrentHealth
        // is truncated to Max without those bonuses and then persisted on logout.
        await Collections.PegasusCollectionService.PrepareAttributesForEnterWorldAsync(this).ConfigureAwait(false);
        SoulSystem.SoulSystemService.ApplyBonuses(this);
