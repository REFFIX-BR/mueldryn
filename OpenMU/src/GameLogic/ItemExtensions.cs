...
public static class ItemExtensions
{
    ...
    /// <summary>
    /// Rolls Dream-style Excellent Bonus Options (Strength / Energy) when the item
    /// is excellent and does not already have them. Called on monster drops, boxes, and mixes.
    /// </summary>
    public static void EnsureExcellentBonusStats(this Item item)
    {
        ...
        var randomizer = Rand.GetRandomizer();
        var dropLevel = item.Definition?.DropLevel ?? 0;
        var baseBonus = Math.Clamp(20 + (dropLevel / 3), 15, 120);

        var both = randomizer.NextRandomBool(0.70);
        if (both || randomizer.NextRandomBool(0.50))
        {
            item.ExcellentBonusStrength = (byte)Math.Clamp(baseBonus + randomizer.NextInt(0, 41), 1, 255);
        }

        if (both || item.ExcellentBonusStrength == 0)
        {
            item.ExcellentBonusEnergy = (byte)Math.Clamp(baseBonus + randomizer.NextInt(0, 51), 1, 255);
        }
    }
