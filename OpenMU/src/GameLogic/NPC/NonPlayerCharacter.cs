...
public class NonPlayerCharacter : AsyncDisposable, IObservable, IRotatable, ILocateable, IHasBucketInformation
{
    ...
    public virtual void Initialize()
    {
        ...
        // Point packs used to retry once and land every Quantity copy on the same tile.
        // Search the configured area first, then a small ring around a point spawn.
        var maxRetry = this.SpawnArea.IsPoint() ? 80 : maxRetryForAreas;
        ...
        if (spawnPoint is null
            && this.SpawnArea.IsPoint()
            && this.Definition.ObjectKind == NpcObjectKind.Monster)
        {
            spawnPoint = this.FindNearbyWalkablePoint(this.SpawnArea.X1, this.SpawnArea.Y1, 2);
        }
    ...
    private bool IsValidSpawnPoint(Point spawnPoint)
    {
        ...
        if (!((isSafezoneAllowed || !isInSafezone) && (!npcCanWalk || isWalkable)))
        {
            return false;
        }

        return this.Definition.ObjectKind != NpcObjectKind.Monster || !this.IsOccupiedByMonster(spawnPoint);
    }

    private bool IsOccupiedByMonster(Point spawnPoint)
    {
        foreach (var npc in this.CurrentMap.GetNpcsInRange(spawnPoint, 1))
        {
            if (npc != this
    ...
    private Point? FindNearbyWalkablePoint(byte originX, byte originY, int radius)
    {
