...
public class ItemStackAction
{
    ...
    public async ValueTask StackItemsAsync(Player player, byte stackId, byte stackSize)
    {
        ...
        var available = player.Inventory.Items
            .Where(item => item.Definition == mix.SingleJewel)
            .Sum(item => Math.Max(1, (int)item.Durability));
        ...
        var remaining = (int)stackSize;
        foreach (var jewel in player.Inventory.Items.Where(item => item.Definition == mix.SingleJewel).ToList())
        {
            if (remaining <= 0)
            {
                break;
            ...
            var have = Math.Max(1, (int)jewel.Durability);
            ...
            {
                jewel.Durability -= take;
                await player.InvokeViewPlugInAsync<IItemDurabilityChangedPlugIn>(p => p.ItemDurabilityChangedAsync(jewel, false)).ConfigureAwait(false);
            }
        }
    ...
    public async ValueTask UnstackItemsAsync(Player player, byte stackId, byte slot)
    {
        ...
        var maxStack = mix.SingleJewel!.Durability > 1 ? (int)mix.SingleJewel.Durability : 1;

        // Prefer returning one flexible inventory stack when the single jewel is stackable.
        if (maxStack > 1 && pieces <= maxStack)
        {
            if (!player.Inventory!.FreeSlots.Any())
            {
            ...
            jewel.Durability = pieces;
            await player.Inventory.AddItemAsync(jewel).ConfigureAwait(false);
