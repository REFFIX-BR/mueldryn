...
public sealed class ItemBoxDroppedPlugIn : IItemDropPlugIn
{
    ...
    public async ValueTask HandleItemDropAsync(Player player, Item sourceItem, Point target, ItemDropArguments cancelArgs)
    {
        ...
        // Box of Kundun (+1..+5): always drop an item, never zen.
        var dropGroups = itemDropGroups.ToList();
        var isBoxOfKundun = sourceItem.Definition.Group == 14
            && sourceItem.Definition.Number == 11
            && sourceItem.Level is >= 8 and <= 12;
        if (isBoxOfKundun)
        {
            dropGroups = dropGroups.Where(g => g.ItemType != SpecialItemType.Money).ToList();
        }

        Item? item = null;
        uint? droppedMoneyAmount = null;
        var dropEffect = ItemDropEffect.Undefined;

        if (isBoxOfKundun)
        {
            // SelectRandomGroup uses absolute chances; with money removed, Chance=0.2 would often miss.
            // Force item generation from the excellent/item groups.
            for (var attempt = 0; attempt < 8 && item is null; attempt++)
            {
                (item, _, dropEffect) = player.GameContext.DropGenerator.GenerateItemDrop(dropGroups);
            }
            ...
            if (item is null)
            {
                ...
                {
                    item = player.GameContext.DropGenerator.GenerateItemDrop(group);
                    if (item is not null)
                    {
                        if (group is ItemDropItemGroup idg)
                        {
                            dropEffect = idg.DropEffect;
                        }
        ...
        {
            (item, droppedMoneyAmount, dropEffect) = player.GameContext.DropGenerator.GenerateItemDrop(dropGroups);
        }
