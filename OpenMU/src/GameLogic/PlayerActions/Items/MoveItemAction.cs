...
public class MoveItemAction
{
    ...
    private async ValueTask<Movement> CanMoveAsync(Player player, Item item, byte toSlot, byte fromSlot, StorageInfo? toStorage, StorageInfo? fromStorage)
    {
        ...
        // Cash/period items: no trade, personal shop, or vault.
        if (item.HasExpiration
            && toStorage.Storage != fromStorage.Storage
            && (toStorage.Storage == player.TemporaryStorage
                || toStorage.Storage == player.ShopStorage
                || toStorage.Storage == player.Vault
                || fromStorage.Storage == player.TemporaryStorage
                || fromStorage.Storage == player.ShopStorage))
        {
