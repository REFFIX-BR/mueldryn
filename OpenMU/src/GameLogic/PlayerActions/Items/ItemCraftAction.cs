...
public class ItemCraftAction
{
    ...
    public async ValueTask MixItemsAsync(Player player, byte mixTypeId, byte socketSlot)
    {
        ...
        crafting ??= this.FindAppropriateCraftingByItems(player);
        if (crafting is null)
        {
        ...
        var craftingHandler = this.GetOrCreateCraftingHandler(crafting);
        if (craftingHandler.TryGetRequiredItems(player, out _, out _) is not null)
        {
            var matchedCrafting = this.FindAppropriateCraftingByItems(player);
            if (matchedCrafting is not null)
            {
                crafting = matchedCrafting;
                craftingHandler = this.GetOrCreateCraftingHandler(crafting);
            }
        }
    ...
    public ItemCrafting? FindAppropriateCraftingByItems(Player player)
    {
        if (player.OpenedNpc?.Definition is { } npc)
        {
            ...
            {
                var craftingHandler = this.GetOrCreateCraftingHandler(itemCrafting);
    ...
    private IItemCraftingHandler CreateCraftingHandler(ItemCrafting crafting)
    {
        if (!string.IsNullOrWhiteSpace(crafting.ItemCraftingHandlerClassName))
        {
            var type = Type.GetType(crafting.ItemCraftingHandlerClassName)
                       ?? typeof(SimpleItemCraftingHandler).Assembly.GetType(crafting.ItemCraftingHandlerClassName);
            if (type != null)
            {
