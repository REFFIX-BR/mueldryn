...
public class BuyRequestAction
{
    ...
    public async ValueTask BuyItemAsync(Player player, Player requestedPlayer, byte slot)
    {
        ...
        if (!PersonalShopPrice.TryDecode(itemPrice, out _))
        {
            await player.InvokeViewPlugInAsync<IPlayerShopBuyRequestResultPlugIn>(
                p => p.ShowResultAsync(requestedPlayer, ItemBuyResult.NameMismatchOrPriceMissing, null)).ConfigureAwait(false);
            return;
        }
        ...
        using (await requestedPlayer.ShopStorage.StoreLock.LockAsync())
        {
            ...
            if (item?.StorePrice is null || item.StorePrice.Value != itemPrice)
            {
                await player.InvokeViewPlugInAsync<IPlayerShopBuyRequestResultPlugIn>(p => p.ShowResultAsync(requestedPlayer, ItemBuyResult.InvalidShopSlot, null)).ConfigureAwait(false);
                return;
            }
            ...
            var payment = await PersonalShopPaymentService.TryTransferAsync(player, requestedPlayer, itemPrice).ConfigureAwait(false);
            if (payment.Result != PersonalShopPaymentResult.Success || payment.Receipt is null)
            {
                var buyResult = payment.Result == PersonalShopPaymentResult.NotEnough
                    ? ItemBuyResult.LackOfMoney
                    : payment.Result == PersonalShopPaymentResult.SellerCapacity
                        ? ItemBuyResult.MoneyOverflowOrNotEnoughSpace
                        : ItemBuyResult.NameMismatchOrPriceMissing;
                await player.InvokeViewPlugInAsync<IPlayerShopBuyRequestResultPlugIn>(
                    p => p.ShowResultAsync(requestedPlayer, buyResult, null)).ConfigureAwait(false);
                return;
            }
            ...
            {
                await payment.Receipt.RollbackAsync().ConfigureAwait(false);
                item.ItemSlot = slot;
                item.StorePrice = itemPrice;
                await requestedPlayer.ShopStorage.AddItemAsync(item).ConfigureAwait(false);
                await player.InvokeViewPlugInAsync<IPlayerShopBuyRequestResultPlugIn>(
                    p => p.ShowResultAsync(requestedPlayer, ItemBuyResult.MoneyOverflowOrNotEnoughSpace, null)).ConfigureAwait(false);
                return;
            }
