...
public class EnterMiniGameAction
{
    ...
    /// <summary>Group of the event tickets which are sold in the item shop.</summary>
    private const byte ShopTicketGroup = 13;

    private static readonly Dictionary<MiniGameType, short> ShopTicketNumbers = new()
    {
        { MiniGameType.DevilSquare, 46 },
        { MiniGameType.BloodCastle, 47 },
        { MiniGameType.IllusionTemple, 61 },
    };
    ...
    private bool CheckTicketItem(MiniGameDefinition miniGameDefinition, Player player, byte gameTicketInventoryIndex, out Item? ticketItem)
    {
        ...
        return this.TryFindShopTicket(miniGameDefinition, player, out ticketItem);
    }

    /// <summary>
    /// Looks for the ticket which the item shop sells for this event. It works for every level of the
    /// event, so unlike the quest ticket it does not have to match <see cref="MiniGameDefinition.TicketItemLevel"/>.
    /// </summary>
    private bool TryFindShopTicket(MiniGameDefinition miniGameDefinition, Player player, out Item? ticketItem)
    {
        ticketItem = null;

        if (!ShopTicketNumbers.TryGetValue(miniGameDefinition.Type, out var shopTicketNumber)
            || player.Inventory is not { } inventory)
        {
            return false;
        }

        ticketItem = inventory.Items.FirstOrDefault(item =>
            item.Definition?.Group == ShopTicketGroup
            && item.Definition.Number == shopTicketNumber
            && item.Durability > 0
            && !item.IsExpirationElapsed);
