...
public class DefaultDropGenerator : IDropGenerator
{
    ...
    /// <summary>Season 1: always 1 excellent option; this is the chance for the 2nd.</summary>
    private const double SecondExcellentOptionChance = 0.35;

    /// <summary>Season 1 cap for excellent options (Dream can go to 3 later).</summary>
    private const int MaximumExcellentOptionsSeason1 = 2;

    /// <summary>Box of Kundun v1: luck is a roll, not guaranteed.</summary>
    private const double BoxLuckChance = 0.25;

    /// <summary>Box of Kundun v1: additional option is always 1 add (+4).</summary>
    private const byte BoxAdditionalOptionLevel = 1;

    /// <summary>Weights for option rarity: Normal, Uncommon, Rare, Epic.</summary>
    private static readonly int[] ExcellentOptionRarityWeights = [50, 30, 15, 5];
    ...
    /// <param name="fromItemBox">True when generating from a dropped box (Kundun, etc.).</param>
    /// <returns>A random excellent item.</returns>
    protected Item? GenerateRandomExcellentItem(int monsterLevel = 0, ICollection<ItemDefinition>? possibleItems = null, bool fromItemBox = false)
    {
        ...
        if (fromItemBox)
        {
            this.ApplyKundunBoxAffixes(item);
        }
        else
        {
            item.HasSkill = item.CanHaveSkill();
            this.AddRandomExcOptions(item);
            this.AddLuckToExcellentItem(item);
        }
    ...
    private Item? GenerateItemDrop(DropItemGroup selectedGroup, ICollection<ItemDefinition> possibleItems)
    {
        var item = selectedGroup.ItemType switch
        {
            ...
            SpecialItemType.Excellent => this.GenerateRandomExcellentItem(
                possibleItems: possibleItems,
                fromItemBox: selectedGroup is ItemDropItemGroup),
    ...
    private void AddRandomExcOptions(Item item, int? maximumOptions = null)
    {
        ...
        var maxOptions = maximumOptions ?? MaximumExcellentOptionsSeason1;

        // Always at least 1 excellent option; extra options are luck-based unless the caller caps them.
        // Level on the link stores option rarity: Normal/Uncommon/Rare/Epic (tooltip tags).
        for (int i = existingOptionCount; i < maxOptions; i++)
        {
            if (i > 0 && !this._randomizer.NextRandomBool(SecondExcellentOptionChance))
            {
                break;
            }

            var available = excellentOptions.PossibleOptions
                .Where(option => item.ItemOptions.All(link => !object.Equals(link.ItemOption, option)))
                .ToList();
            ...
            var newOption = available.SelectRandom(this._randomizer);
            if (newOption is not null)
            {
                item.ItemOptions.Add(new ItemOptionLink
                {
                    ItemOption = newOption,
                    Level = (byte)this.RollExcellentOptionRarity(),
                });
            }
        }
    }

    private void ApplyKundunBoxAffixes(Item item)
    {
        foreach (var link in item.ItemOptions
            .Where(option => object.Equals(option.ItemOption?.OptionType, ItemOptionTypes.Luck)
                             || object.Equals(option.ItemOption?.OptionType, ItemOptionTypes.Option))
            .ToList())
        {
            item.ItemOptions.Remove(link);
        }

        this.AddRandomExcOptions(item, maximumOptions: 1);
        this.AddAdditionalOption(item, BoxAdditionalOptionLevel);

        if (this._randomizer.NextRandomBool(BoxLuckChance))
        {
            this.AddLuckToExcellentItem(item);
        }
    ...
    private void AddAdditionalOption(Item item, byte optionLevel)
    {
        if (optionLevel == 0
            || item.ItemOptions.Any(option => object.Equals(option.ItemOption?.OptionType, ItemOptionTypes.Option)))
        {
            return;
        }

        var additionalOption = item.Definition!.PossibleItemOptions
            .SelectMany(definition => definition.PossibleOptions)
            .FirstOrDefault(option => object.Equals(option.OptionType, ItemOptionTypes.Option));
        if (additionalOption is null)
        {
            return;
        }
    ...
    private ExcellentOptionRarity RollExcellentOptionRarity()
    {
        var total = ExcellentOptionRarityWeights.Sum();
        var roll = this._randomizer.NextInt(0, total);
        var cumulative = 0;
        for (var i = 0; i < ExcellentOptionRarityWeights.Length; i++)
        {
            cumulative += ExcellentOptionRarityWeights[i];
            if (roll < cumulative)
            {
                return (ExcellentOptionRarity)i;
            }
        }

        return ExcellentOptionRarity.Normal;
    }

    private void AddLuckToExcellentItem(Item item)
    {
        if (item.ItemOptions.Any(o => object.Equals(o.ItemOption?.OptionType, ItemOptionTypes.Luck)))
        {
            return;
        }
