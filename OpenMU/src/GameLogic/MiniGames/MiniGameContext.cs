...
public class MiniGameContext : AsyncDisposable, IEventStateProvider
{
    ...
    /// <summary>
    /// Gets the countdown duration used before start/exit of the mini game.
    /// </summary>
    protected virtual TimeSpan CountdownDuration => TimeSpan.FromSeconds(30);

    /// <summary>
    /// Gets how long the game should run after it started.
    /// </summary>
    protected virtual TimeSpan GameDuration => this.Definition.GameDuration;

    /// <summary>
    /// Gets a custom death respawn gate for this mini game, or <c>null</c> to use the map spawn.
    /// </summary>
    public virtual ExitGate? GetPlayerRespawnGate() => null;
    ...
    /// <summary>
    /// Gets a value indicating whether NPCs tagged for event start should spawn.
    /// Dungeon waves are spawned in code, so the dungeon instance disables this.
    /// </summary>
    protected virtual bool ShouldInitializeEventStartNpcs => true;

    /// <summary>
    /// Gets a value indicating whether configured <see cref="MiniGameDefinition.SpawnWaves"/> should run.
    /// </summary>
    protected virtual bool ShouldRunDefinitionSpawnWaves => true;
    ...
    /// <summary>
    /// Gets a value indicating whether a player who left the mini-game map should be detached from the instance.
    /// Death respawns stay attached so wave monsters are not disposed.
    /// </summary>
    protected virtual bool ShouldDetachPlayerOnMapRemove(Player player) => true;
    ...
    protected virtual async ValueTask OnObjectRemovedFromMapAsync((GameMap Map, ILocateable Object) args)
    {
        try
        {
            ...
            if (!this.ShouldDetachPlayerOnMapRemove(player))
            {
                return;
            }
    ...
    private async ValueTask RunGameAsync(CancellationToken cancellationToken)
    {
        ...
        var countdownMessageDuration = this.CountdownDuration;
        try
        {
            var enterDuration = this.Definition.EnterDuration.AtLeast(countdownMessageDuration);
            var gameDuration = this.GameDuration.AtLeast(countdownMessageDuration);
    ...
    private async ValueTask MovePlayersToSafezoneAsync()
    {
        ...
        {
            await this.WarpPlayerOutAsync(player).ConfigureAwait(false);
        }
    }

    /// <summary>
    /// Warps a player out of the mini game when it ends or times out.
    /// </summary>
    protected virtual async ValueTask WarpPlayerOutAsync(Player player)
    {
        await player.WarpToSafezoneAsync().ConfigureAwait(false);
    }

    /// <summary>
    /// Detaches the player from this instance and warps them out, without
    /// sending remaining participants with them.
    /// </summary>
    protected async ValueTask DetachAndWarpOutAsync(Player player)
    {
        player.CurrentMiniGame = null;
        player.PlayerPickedUpItem -= this.OnPlayerPickedUpItemAsync;
        bool cantGameProceed;
