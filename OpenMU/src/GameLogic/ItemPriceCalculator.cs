...
public class ItemPriceCalculator
{
    ...
    private static readonly IDictionary<int, Func<Item, long>> SpecialItemDictionary = new Dictionary<int, Func<Item, long>>
    {
        ...
        { (int)SpecialItems.Bless, item => StackedPieces(item) * 9000000 },
        { (int)SpecialItems.Soul, item => StackedPieces(item) * 6000000 },
        { (int)SpecialItems.Chaos, item => StackedPieces(item) * 810000 },
        { (int)SpecialItems.Life, item => StackedPieces(item) * 45000000 },
        { (int)SpecialItems.Creation, item => StackedPieces(item) * 36000000 },
        { (int)SpecialItems.Guardian, item => StackedPieces(item) * 60000000 },
        { (int)SpecialItems.Gemstone, item => StackedPieces(item) * 18600 },
        { (int)SpecialItems.Harmony, item => StackedPieces(item) * 18600 },
        { (int)SpecialItems.LowerRefineStone, item => StackedPieces(item) * 18600 },
        { (int)SpecialItems.HigherRefineStone, item => StackedPieces(item) * 18600 },
    ...
    public long CalculateFinalBuyingPrice(Item item)
    {
        // Merchant items can pin an explicit zen price (e.g. collection test gear at 1 zen).
        if (item.StorePrice is > 0)
        {
            return item.StorePrice.Value;
        }

        return RoundPrice(CalculateBuyingPrice(item));
    }
    ...
    /// <summary>
    /// Number of pieces held by a stackable item. Non-wearable items keep the piece count in
    /// <see cref="Item.Durability"/>, so a stack of nine jewels is worth nine single ones.
    /// </summary>
    private static long StackedPieces(Item item)
    {
        return Math.Max(1, (long)item.Durability());
    }
