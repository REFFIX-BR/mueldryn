...
public abstract class BaseInvasionPlugIn<TConfiguration> : PeriodicTaskBasePlugIn<TConfiguration, InvasionGameServerState>, IPeriodicTaskPlugIn, IObjectAddedToMapPlugIn, ISupportCustomConfiguration<TConfiguration>, IProvidesEventSchedule, IProvidesInvasionMonsterStatus
    where TConfiguration : PeriodicInvasionConfiguration
{
    /// <inheritdoc />
    public abstract string ScheduleDisplayName { get; }

    /// <inheritdoc />
    public EventScheduleEntry? TryGetScheduleEntry(IGameContext gameContext)
    {
        var configuration = this.Configuration;
        if (configuration is null && this is ISupportDefaultCustomConfiguration defaultConfigSupporter)
        {
            configuration = defaultConfigSupporter.CreateDefaultConfig() as TConfiguration;
        }

        var state = this.GetStateByGameContext(gameContext);
        var computed = EventScheduleTiming.TryCompute(
            configuration,
            state.State,
            state.LastRunUtc,
            state.NextRunUtc,
        ...
        {
            Name = this.ScheduleDisplayName,
            Category = EventScheduleCategory.Invasion,
            IsActive = isActive,
    ...
    /// <inheritdoc />
    public IReadOnlyList<InvasionMonsterStatusEntry> GetMonsterStatusEntries(IGameContext gameContext)
    {
        var state = this.GetStateByGameContext(gameContext);
        if (state.State is PeriodicTaskState.NotStarted || !state.HasMonsterStatus)
        {
            return [];
        }

        return state.GetMonsterStatusRows()
            .Select(row => new InvasionMonsterStatusEntry
            {
                InvasionName = this.ScheduleDisplayName,
    ...
    /// <inheritdoc />
    public int? TryGetActiveSecondsRemaining(IGameContext gameContext)
    {
        var configuration = this.Configuration;
        if (configuration is null && this is ISupportDefaultCustomConfiguration defaultConfigSupporter)
        {
        ...
        var state = this.GetStateByGameContext(gameContext);
        if (state.State is PeriodicTaskState.NotStarted)
        {
            return null;
        }
        ...
        var computed = EventScheduleTiming.TryCompute(
            ...
            state.NextRunUtc,
            DateTime.UtcNow);
        return computed?.SecondsRemaining;
    }
    ...
    protected async ValueTask SpawnMobsAsync(IGameContext gameContext, ushort mapId, IEnumerable<InvasionSpawnConfiguration> spawns)
    {
        ...
        foreach (var spawn in spawns)
        {
            ...
            {
                var state = this.GetStateByGameContext(gameContext);
                state.AddPlannedSpawn(
                    spawn.MonsterId,
                    monsterDefinition.Designation.ToString() ?? $"Monster {spawn.MonsterId}",
                    spawn.Count);
