...
public class PlayerLosesExperienceAfterDeathPlugIn : IAttackableGotKilledPlugIn, ISupportCustomConfiguration<PlayerLosesExperienceAfterDeathPlugInConfiguration>, ISupportDefaultCustomConfiguration
{
    ...
    public async ValueTask AttackableGotKilledAsync(IAttackable killed, IAttacker? killer)
    {
        ...
        if (await TryConsumeResurrectionTalismanAsync(player).ConfigureAwait(false))
        {
            return;
        }
    ...
    /// <summary>
    /// Uses up one talisman of resurrection of the item shop, if the player carries one. It saves the
    /// experience which would be lost by this death.
    /// </summary>
    private static async ValueTask<bool> TryConsumeResurrectionTalismanAsync(Player player)
    {
        const byte talismanGroup = 13;
        const short talismanNumber = 69;
        ...
        var talisman = inventory.Items.FirstOrDefault(item =>
            item.Definition?.Group == talismanGroup
            && item.Definition.Number == talismanNumber
            && item.Durability > 0
        ...
        talisman.Durability -= 1;
        if (talisman.Durability == 0)
        {
            await player.DestroyInventoryItemAsync(talisman).ConfigureAwait(false);
        }
        else
        {
            await player.InvokeViewPlugInAsync<IItemDurabilityChangedPlugIn>(p => p.ItemDurabilityChangedAsync(talisman, false)).ConfigureAwait(false);
        }
