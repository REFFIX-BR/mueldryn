...
public class ResetCharacterAction
{
    ...
    public async ValueTask ResetCharacterAsync()
    {
        ...
        // Experience is cumulative: being at level N requires ExperienceTable[N], not 0.
        // Leaving it at 0 forces grinding the full sum of levels 1..N again and desyncs the client bar.
        var levelAfterReset = Math.Clamp(configuration.LevelAfterReset, 0, this._player.GameContext.ExperienceTable.Length - 1);
        this._player.SelectedCharacter.Experience = this._player.GameContext.ExperienceTable[levelAfterReset];
        this.UpdateStats(configuration, resetProgression);
        MUnique.OpenMU.GameLogic.SoulSystem.SoulSystemService.GrantResetReward(this._player);
    ...
    private void UpdateStats(ResetConfiguration configuration, ResetProgression resetProgression)
    {
        ...
        var questBonusPerReset = QuestPanelService.GetPermanentPointBonus(this._player);

        if (configuration.ReplacePointsPerReset)
        {
            var questBonusTotal = (long)questBonusPerReset * resetProgression.NextResetCount;
            var basePoints = resetProgression.TotalPointsAfterReset;
            this._player.SelectedCharacter!.LevelUpPoints = (int)Math.Min(int.MaxValue, basePoints + questBonusTotal);
        }
        else
        {
            this._player.SelectedCharacter!.LevelUpPoints += resetProgression.PointsForReset + questBonusPerReset;
        }
    }
