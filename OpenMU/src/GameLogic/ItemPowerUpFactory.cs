...
public class ItemPowerUpFactory : IItemPowerUpFactory
{
    ...
    public IEnumerable<PowerUpWrapper> GetPowerUps(Item item, AttributeSystem attributeHolder)
    {
        ...
        if (item.ExcellentBonusStrength > 0)
        {
            yield return new PowerUpWrapper(
                new SimpleElement(item.ExcellentBonusStrength, AggregateType.AddRaw),
                Stats.TotalStrength,
                attributeHolder);
        }

        if (item.ExcellentBonusEnergy > 0)
        {
            yield return new PowerUpWrapper(
                new SimpleElement(item.ExcellentBonusEnergy, AggregateType.AddRaw),
                Stats.TotalEnergy,
                attributeHolder);
        }
    ...
    private IEnumerable<PowerUpWrapper> GetPowerUpsOfItemOptions(Item item, AttributeSystem attributeHolder)
    {
        ...
        foreach (var optionLink in options)
        {
            ...
            // Excellent option rarity is stored in ItemOptionLink.Level (0..3).
            // Do not treat that as LevelDependentOptions level.
            var isExcellent = option.OptionType == ItemOptionTypes.Excellent;
            var level = option.LevelType == LevelType.ItemLevel
                ? item.Level
                : isExcellent ? 0 : optionLink.Level;
            ...
            if (isExcellent)
            {
                ...
                var rarity = (ExcellentOptionRarity)Math.Clamp((int)optionLink.Level, 0, 3);
                var scaled = ExcellentOptionRarityValues.TryGetBoostValue(option, rarity);
                if (scaled is { } scaledValue && powerUp.Boost.ConstantValue is { } constant)
                {
                    yield return new PowerUpWrapper(
                        new SimpleElement(scaledValue, constant.AggregateType),
                        powerUp.TargetAttribute!,
                        attributeHolder,
                        aggregateType);
                    continue;
                }
