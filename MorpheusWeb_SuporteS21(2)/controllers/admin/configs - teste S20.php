...
Route::map("/configs/general", function () {
    if (Request::isPost()) {
        try {
            ...
            $item_file_uploaded = null;
            $storage_type = null;

            $items_igcn = Input::file("item_igcn");
            $items_kor = Input::file("itemkor");

            // Prioriza o arquivo IGCN se ele for enviado
            if (!empty($items_igcn)) {
                $item_file_uploaded = $items_igcn;
                $storage_type = "IGCN";
            } elseif (!empty($items_kor)) {
                $item_file_uploaded = $items_kor;
                    $storage_type = "ItemKOR";
            }

            if ($item_file_uploaded) {
                $dir = ROOT . DS . "resources" . DS . "files" . DS;
                list($file) = rework_upload_files($item_file_uploaded);
                $storage = Morpheus\Item\Storage\Storage::factory($storage_type, $file["tmp_name"]);
    ...
    $teams = array("xteam" => "X-Team", "muemu" => "MuEmu/LouisEmulator", "gx" => "GXGaming", "igcn" => "IGCN/Season 20", "gmo" => "GMO");
