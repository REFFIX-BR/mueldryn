...
class License
{
    ...
    public function update()
    {
        ...
        $data = $this->getData();
        $http = \Morpheus\Network\Http::create("GET", LICENSE_SERVER_URL . "license/update", array("query" => array("mac" => $this->getMacAddress(), "ip" => join(",", $this->getIpAddress()), "domain" => $this->_server["SERVER_NAME"], "path" => config("base_path"), "template" => config("template", "default"), "version" => MORPHEUS_VERSION, "php_version" => phpversion(), "id" => isset($data["id"]) ? $data["id"] : "")))->send();
        ...
        if (is_array($data)) {
            if (isset($data["status"])) {
                ...
                if ($domain && ($mac || $allowIp)) {
                    ...
                    $version = substr(MORPHEUS_VERSION, 0, 1);
            ...
            switch ($data["result"]) {
                ...
                case "CANCELED":
                    return app()->redirect(MORPHEUS_WEBSITE . "license-canceled");
            }
        } else {
            return app()->redirect(MORPHEUS_WEBSITE . "license-invalid");
        }*/
