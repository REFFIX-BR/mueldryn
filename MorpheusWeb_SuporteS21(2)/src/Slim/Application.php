...
class Application extends \Slim\Slim
{
    ...
    public function redirect($url, $status = 302)
    {
        if (filter_var($url, FILTER_VALIDATE_URL) === false) {
            parent::redirect(url_to($url, true), $status);
        } else {
    ...
    {
        $dirs = array_filter(glob(LANGUAGE_PATH . "*"), "is_dir");
        $languages = array();
        foreach ($dirs as $dir) {
            $parts = explode(DS, $dir);
            $dir = array_pop($parts);
            if (isset($this->_locales[$dir])) {
                $languages[$dir] = __($this->_locales[$dir]);
            }
        }
        if (!isset($languages["en_US"])) {
            $languages["en_US"] = __($this->_locales["en_US"]);
        }
        ...
        $this->view()->replace(array("languages" => $languages, "locale" => $lang ?: config("locale", "pt_BR")));
    }
    public function hook($name, $callable, $priority = 10)
    {
        if (is_array($name)) {
            foreach ($name as $hook) {
                parent::hook($hook, $callable, $priority);
            }
        } else {
