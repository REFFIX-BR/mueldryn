...
class Custom extends AbstractRanking
{
    protected $_view = "rankings/custom";
    public function __construct(array $params = array())
    {
        parent::__construct($params);
        $name = isset($params["name"]) ? $params["name"] : "";
        $key = "rankings." . $name . ".configs";
        $configs = config($key, array());
        $this->_table = config($key . ".table");
        $this->_score = config($key . ".score_column");
        ...
        $score = ($this->_table === "Character" ? "c." : "ds.") . $this->_score;
        ...
        if ($this->_table == "Character") {
            ...
            $qb->from($this->_table, "ds")->leftJoin("ds", "Character", "c", "ds." . config("rankings." . $this->_params["name"] . ".configs.character_column") . " = c.Name");
        }
