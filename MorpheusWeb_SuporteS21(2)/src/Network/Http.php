...
class Http
{
    protected $options = array("handler" => NULL, "method" => "GET", "url" => "", "nobody" => false, "follow_redirects" => 0, "protocol_version" => "1.1", "timeout" => 10, "user_agent" => "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.9; rv:38.0) Gecko/20100101 Firefox/38.0", "auth" => NULL, "proxy" => NULL, "proxy_userpwd" => NULL, "proxy_type" => "http", "headers" => array(), "cookies" => array(), "json" => false, "body" => "", "query" => array(), "form_params" => array(), "multipart" => array(), "curl" => array());
    public $request = NULL;
    public $response = NULL;
    protected $redirects = array("count" => 0, "urls" => array(), "cookies" => array(), "requests" => array());
    protected $debugInfo = array("time_start" => NULL, "time_process" => NULL, "handler" => NULL, "errors" => array());
    private $builder = array("headers" => array(), "query" => array(), "form_params" => array());
    ...
    public static function create($url, $method = "GET", $options = array())
    {
        ...
        $object = new self();
        $params = array("form_params" => "withFormParam", "query" => "withQuery");
    ...
    public function send()
    {
        ...
        if ($info !== false) {
            $this->response = array("header" => $info[0], "body" => $info[1]);
    ...
    {
        $client = clone $this;
        while (($this->options["follow_redirects"] === true || $this->redirects["count"] < $this->options["follow_redirects"]) && ($nextUrl = $client->getResponseHeaderLine("Location"))) {
            ...
            $reuseOptions = array("nobody", "protocol_version", "timeout", "user_agent", "auth", "headers", "proxy", "proxy_userpwd", "proxy_type", "query");
    ...
    private function collectResponseCookies(&$collection, $cookies)
    {
        ...
        foreach ($collection as $oldKey => $oldValue) {
            foreach ($cookies as $newKey => $newValue) {
                if ($oldValue["Name"] === $newValue["Name"] && $oldValue["Path"] === $newValue["Path"] && $oldValue["Domain"] === $newValue["Domain"] && $oldValue["Secure"] === $newValue["Secure"] && $oldValue["HttpOnly"] === $newValue["HttpOnly"]) {
    ...
    {
        return $this->getRedirectedCount() ? end($this->redirects["urls"]) : $this->options["url"];
    }
    public function getAllResponseCookies()
    {
        return $this->getRedirectedCount() ? $this->getRedirectedCookies() : $this->getResponseArrayCookies();
    }
    ...
    protected function sendWithSocket(array $request)
    {
        ...
        $message .= "\r\n\r\n";
        $errorHandler === null and create_function("", "");
    ...
    {
        $curlOptions = array(CURLOPT_CUSTOMREQUEST => $request["method"], CURLOPT_URL => $request["uri"], CURLOPT_HEADER => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_ENCODING => "gzip, deflate", CURLOPT_NOBODY => $this->options["nobody"], CURLOPT_TIMEOUT => $this->options["timeout"], CURLOPT_HTTPHEADER => $request["headers"]);
    ...
    protected function parseResponseHeaders($headers)
    {
        ...
        $out = array("protocol_version" => $match[1], "status" => (int) $match[2], "reason" => $match[3], "headers" => array());
    ...
    public function prepareRequest()
    {
        ...
        $request = array("protocol_version" => $clone->options["protocol_version"], "method" => strtoupper($clone->options["method"]), "uri" => $clone->getUriAsString($uri), "uriInfo" => $uri);
        $clone->options["url"] = $request["uri"];
        if ($clone->options["json"]) {
            !$clone->hasHeader("Content-Type") and $clone->withHeader("Content-Type", "application/json");
        ...
        if ($clone->options["multipart"]) {
            $boundary = uniqid();
            !$clone->hasHeader("Content-Type") and $clone->withHeader("Content-Type", "multipart/form-data; boundary=" . $boundary);
            ...
            if ($params) {
                !$clone->hasHeader("Content-Type") and $clone->withHeader("Content-Type", "application/x-www-form-urlencoded");
                $body .= $params;
            }
        }
        ...
        $clone->options["auth"] and $clone->withHeader("Authorization", "Basic " . base64_encode($clone->options["auth"]));
        $cookies = "";
        ...
        foreach ($clone->options["cookies"] as $cookie) {
            if ($clone->isCookieMatchesDomain($cookie, $uri["host"]) && $clone->isCookieMatchesPath($cookie, $uri["path"])) {
                $cookies .= $clone->getCookieAsString($cookie, false);
            }
        }
        $cookies and $clone->withHeader("Cookie", trim($cookies), false);
        $body and $clone->withHeader("Content-Length", strlen($body), false);
        $clone->withHeader("User-Agent", $clone->options["user_agent"], false);
        !$clone->hasHeader("Expect") and !$clone->hasHeader("Expect") && $body and $clone->withHeader("Expect", "");
        ...
        $request += array("headers" => $headers, "body" => $body);
    ...
    public function withProxy($proxy, $userPwd = NULL, $type = "http")
    {
        if ($proxy === null || preg_match("#^\\d+\\.\\d+\\.\\d+\\.\\d+:\\d+\$#", $proxy)) {
            return $this->setOptions("proxy", $proxy)->setOptions("proxy_userpwd", $userPwd)->setOptions("proxy_type", $type);
        }
    ...
    public function withJson($json)
    {
        ...
        $this->options["json"] = json_encode($json);
    ...
    {
        static $validProtocolVersions = array("1.0" => true, "1.1" => true, "2.0" => true);
    ...
    {
        $append = array("Path" => $path, "Secure" => $secure, "HttpOnly" => $httpOnly);
    ...
    public function withCookie($name, $value = NULL, $path = "/", $secure = false, $httpOnly = false)
    {
        ...
        if (is_array($name)) {
            ...
            if (is_string($name) && $value === null) {
                ...
                if (is_array($value) && $this->isCookieData($value)) {
    ...
    protected function addParamToArray(&$builder, $name, $value = NULL, $append = true)
    {
        if ($value !== null) {
            ...
            if (is_array($name)) {
                ...
                if (is_string($name)) {
                    $name = str_replace("+", "%2B", preg_replace_callback("#&[a-z]+;#", create_function("\$match", "return rawurlencode(\$match[0]);"), $name));
    ...
    protected function addHeaderToArray(&$builder, $name, $values, $append = true)
    {
        if (is_array($name)) {
            ...
            foreach ((array) $values as $value) {
                ...
                $builder[$normalizedKey][] = array("key" => $name, "value" => trim($value));
            }
        }
    }
    ...
    protected function getParamsAsString(array $params)
    {
        ...
        return preg_replace_callback("#([^=&]+)=([^&]*)#i", create_function("\$match", "return \$match[1].\"=\".rawurlencode(urldecode(\$match[2]));"), http_build_query($params));
    }
    ...
    {
        static $defauls = array("Name" => NULL, "Value" => NULL, "Domain" => NULL, "Path" => "/", "Max-Age" => NULL, "Expires" => NULL, "Secure" => false, "Discard" => false, "HttpOnly" => false);
        return $defauls;
    }
    public function parseStringCookies($values)
    {
        ...
        if (preg_match_all("#(?:^|;)\\s*([^=]+)=([^;]+)\\s*?#", $values, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                if (!strcasecmp($name, "Expires") && strtotime($value) || !strcasecmp($name, "Path") && urldecode($value) == $value || preg_match("#Domain|Max-Age|Secure|Discard|HttpOnly#i", $value)) {
                    continue;
                }
        ...
        foreach ($cookie as $key => $value) {
            if ($key != "Name" && $key != "Value" && $value !== null && $value !== false) {
                if ($key == "Expires") {
                    ...
                    $str .= ($value === true ? $key : (string) $key . "=" . $value) . "; ";
                }
            }
        }
        return rtrim($str, "; ");
    ...
    protected function getMultipartAsString($boundary, $parts)
    {
        ...
        foreach ($parts as $field) {
            ...
            $headers["Content-Disposition"] = "form-data; name=\"" . $field["name"] . "\"" . ($field["filename"] ? "; filename=\"" . $field["filename"] . "\"" : "");
    ...
    protected function getFileType($filePath)
    {
        ...
        if (class_exists("finfo", false) && ($info = new finfo(defined("FILEINFO_MIME_TYPE") ? FILEINFO_MIME_TYPE : FILEINFO_MIME))) {
    ...
    protected function getAbsoluteUrl($relative, $base)
    {
        ...
        $relative[0] == "/" and $absolute = $host . $path . "/" . $relative;
        $patterns = array("#(/\\.?/)#", "#/(?!\\.\\.)[^/]+/\\.\\./#");
        $count = 1;
        while (0 < $count) {
            $absolute = preg_replace($patterns, "/", $absolute, -1, $count);
        }
    ...
    protected function getHandler()
    {
        ...
        if ($available === null) {
            $available = array("socket" => function_exists("fsockopen") && function_exists("ini_get") && ini_get("allow_url_fopen"), "curl" => function_exists("curl_init"));
        }
